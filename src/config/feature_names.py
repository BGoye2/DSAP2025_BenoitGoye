...
def get_all_display_names(feature_codes: List[str]) -> Dict[str, str]:
    """
    Get display names for a list of feature codes.
    ...
    --------
    dict
        Mapping from feature codes to display names

    Examples:
    ---------
    >>> codes = ['EG.ELC.ACCS.RU.ZS', 'trade_openness']
    >>> get_all_display_names(codes)
    {'EG.ELC.ACCS.RU.ZS': 'Rural electricity access',
     'trade_openness': 'Trade openness'}
    """
    return {code: get_display_name(code) for code in feature_codes}


def validate_feature_coverage(feature_codes: List[str]) -> Dict[str, any]:
    """
    Check which feature codes have mappings and which don't.

    Parameters:
    -----------
    feature_codes : list
        List of feature codes to validate
    ...
    Examples:
    ---------
    >>> codes = ['EG.ELC.ACCS.RU.ZS', 'UNKNOWN_CODE']
    >>> validate_feature_coverage(codes)
    {'mapped': ['EG.ELC.ACCS.RU.ZS'], 'unmapped': ['UNKNOWN_CODE']}
    """
    mapped = []
    unmapped = []

    for code in feature_codes:
        if code in FEATURE_NAME_MAPPING:
            mapped.append(code)
        else:
    ...
    return {
        'mapped': mapped,
        'unmapped': unmapped,
        'coverage': len(mapped) / len(feature_codes) if feature_codes else 0
    }
